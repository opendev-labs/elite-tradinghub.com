...
export function SiteHeader() {
  ...
  // ── Google Identity Services One Tap (silent account detection & fallback) ──
  useEffect(() => {
    ...
    if (fbUser || session || localSession || stored || clientStored || adminStored) {
      setShowGooglePrompt(false);
      return;
    }
          ...
          callback: async (response: { credential: string }) => {
            ...
            try {
              ...
              if (res.user) {
                ...
                await handleGoogleSignInPrompt();
              }
            } catch (e) {
    ...
    if ((window as any).google?.accounts?.id) {
      ...
      script.onerror = () => setShowGooglePrompt(true);
  ...
  return (
    <>
      ...
      {/* ── Custom Floating Google Sign-In Prompt Card (Top Right) ── */}
      <AnimatePresence>
        {!isLoggedIn && showGooglePrompt && (
          <motion.div
            ...
            className="fixed top-20 right-4 sm:right-6 z-[99999] max-w-sm w-full bg-zinc-900/95 border border-zinc-700/80 shadow-2xl rounded-2xl p-4 backdrop-blur-xl font-sans"
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center p-2 shadow-sm border border-zinc-200 shrink-0">
                ...
                <div className="flex flex-col min-w-0">
                  <h4 className="text-xs font-bold text-zinc-100 truncate">Sign in to elite-tradinghub.com</h4>
                  <p className="text-[10px] text-zinc-400 truncate">Fast & secure access with Google</p>
                </div>
              </div>
              <button
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-zinc-950 font-bold text-xs shadow-lg hover:shadow-emerald-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isConnecting ? (
                <>
                ...
                <>
                  <span>Continue as {detectedGoogleUser?.name ? detectedGoogleUser.name.split(' ')[0] : 'Trader'}</span>
