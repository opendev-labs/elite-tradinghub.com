...
export function AuthLoginScreen({
  ...
  const [email, setEmail] = useState(portalType === "admin" ? "admin@elitetradinghub.com" : "");
    ...
    <div className="min-h-screen w-full grid grid-cols-1 md:grid-cols-2 bg-white text-zinc-900 font-sans">
      ...
      <div className="hidden md:flex flex-col items-center justify-center bg-[#18181b] text-white p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center text-center max-w-md">
          ...
          {portalType === "admin" && (
            <div className="mt-8 px-4 py-2 rounded-full bg-zinc-800/60 border border-zinc-700/50 text-xs text-zinc-400 font-mono">
              Admin Gateway · Demo: yash / 123123
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col items-center justify-center p-6 sm:p-12 bg-white relative">
        <div className="w-full max-w-[360px] mx-auto space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-zinc-700 mb-1.5">
                {portalType === "admin" ? "Username or Email" : "Email Address"}
              </label>
              <input
                type={portalType === "admin" ? "text" : "email"}
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder={portalType === "admin" ? "yash or admin@elitetradinghub.com" : "you@example.com"}
                required
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-10 bg-white hover:bg-zinc-100 text-zinc-950 border border-zinc-300 font-semibold text-xs rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
            >
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-10 bg-white hover:bg-zinc-50 border border-zinc-200 text-zinc-800 font-medium text-xs rounded-lg transition-all shadow-sm flex items-center justify-center gap-2.5"
          >
